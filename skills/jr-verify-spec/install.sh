#!/usr/bin/env bash
# ============================================================
# install-jr-verify-spec.sh
# Instala el skill jr-verify-spec y el comando /jr-verify-spec
...
log()   { echo -e "${BLUE}[jr-verify-spec]${RESET} $*"; }
ok()    { echo -e "${GREEN}✓${RESET} $*"; }
warn()  { echo -e "${YELLOW}⚠${RESET}  $*"; }
error() { echo -e "${RED}✗${RESET} $*" >&2; exit 1; }

CLAUDE_SKILLS_DIR="${HOME}/.claude/skills"
SKILL_DEST="${CLAUDE_SKILLS_DIR}/jr-verify-spec"
CLAUDE_COMMANDS_DIR="${HOME}/.claude/commands"
COMMAND_DEST="${CLAUDE_COMMANDS_DIR}/jr-verify-spec.md"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║     jr-verify-spec — Instalador local        ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${RESET}"
echo ""
...
log "Instalando comando /jr-verify-spec..."
...
if [[ $ERRORS -eq 0 ]]; then
  ...
  echo -e "  ${BLUE}/jr-verify-spec @specs/nombre-del-spec.md${RESET}"
  echo ""

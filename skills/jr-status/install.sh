...
log()   { echo -e "${BLUE}[jr-status]${RESET} $*"; }
ok()    { echo -e "${GREEN}✓${RESET} $*"; }
warn()  { echo -e "${YELLOW}⚠${RESET}  $*"; }
error() { echo -e "${RED}✗${RESET} $*" >&2; exit 1; }

SKILL_DEST="${HOME}/.claude/skills/jr-status"
COMMAND_DEST="${HOME}/.claude/commands/jr-status.md"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║       jr-status — Instalador local           ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${RESET}"
echo ""
...
if [[ $ERRORS -eq 0 ]]; then
  ...
  echo ""
  echo -e "  ${BOLD}Uso:${RESET} ${BLUE}/jr-status${RESET}"

...
log()   { echo -e "${BLUE}[jr-iterate-spec]${RESET} $*"; }
ok()    { echo -e "${GREEN}✓${RESET} $*"; }
warn()  { echo -e "${YELLOW}⚠${RESET}  $*"; }
error() { echo -e "${RED}✗${RESET} $*" >&2; exit 1; }

SKILL_DEST="${HOME}/.claude/skills/jr-iterate-spec"
COMMAND_DEST="${HOME}/.claude/commands/jr-iterate-spec.md"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║    jr-iterate-spec — Instalador local        ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${RESET}"
echo ""
...
if [[ $ERRORS -eq 0 ]]; then
  ...
  echo ""
  echo -e "  ${BOLD}Uso:${RESET} ${BLUE}/jr-iterate-spec @specs/nombre.md${RESET}"

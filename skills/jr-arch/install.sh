...
log()   { echo -e "${BLUE}[jr-arch]${RESET} $*"; }
ok()    { echo -e "${GREEN}✓${RESET} $*"; }
warn()  { echo -e "${YELLOW}⚠${RESET}  $*"; }
error() { echo -e "${RED}✗${RESET} $*" >&2; exit 1; }
SKILL_DEST="${HOME}/.claude/skills/jr-arch"
COMMAND_DEST="${HOME}/.claude/commands/jr-arch.md"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -f "${SCRIPT_DIR}/SKILL.md" ]]   || error "No se encontró SKILL.md"
[[ -f "${SCRIPT_DIR}/command.md" ]] || error "No se encontró command.md"
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║      jr-arch — Instalador local         ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${RESET}"
echo ""

...
| 1.11.0 | Dev mode completed across the lifecycle (Phases 2+3 combined): jr-verify-spec --dev (executable E2E per AC + negative cases, manual per-AC signoff, Verification Quality score ≥85 to flip Verified, evidence persisted in spec), jr-fix-spec --dev (bug report quality scoring with targeted follow-up questions), jr-iterate-spec --dev (3-question iteration justification with push-back). No new skills — 14 total. |
| 1.12.0 | New jr-drift skill (read-only): detects spec-to-code divergence in 3 types — coverage / spec / plan drift. Plus cross-reading across patch/fix/verify/iterate/status (auto `## Post-verification changes`, scope checks, drift awareness). 15 skills total. |

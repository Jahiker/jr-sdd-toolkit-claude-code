...
**jr-toolkit** is a Spec-Driven Development (SDD) toolkit that installs Claude Code skills via `npx`. It covers the full development lifecycle: from raw idea → vision → architecture → roadmap → spec → implementation → verification → bug fixing, plus drift reconciliation.
...
Specs live in `specs/[feature].md`. Bug specs live in `specs/fixes/`. Project context is stored in `PROJECT.md` (with the `## Toolkit Context (jr-toolkit)` block read by every skill at session start). Code generated by `jr-exe-spec` includes `// spec: specs/[name].md` traceability comments.
